...
public class CryptoService {
    ...
    public byte[] wrapKeyWithRsa(SecretKey aesKey, java.security.PublicKey peerPublic) throws Exception {
    ...
    public SecretKey unwrapKeyWithRsa(byte[] wrapped, java.security.PrivateKey ourPrivate) throws Exception {
        Cipher cipher = Cipher.getInstance(RSA_OAEP);
        OAEPParameterSpec oaep = new OAEPParameterSpec("SHA-256","MGF1", new MGF1ParameterSpec("SHA-256"), PSource.PSpecified.DEFAULT);
    ...
    public byte[] sign(byte[] data, java.security.PrivateKey pk) throws Exception {
        Signature s = Signature.getInstance(SIGN_ALGO);
        s.initSign(pk);
        s.update(data);
        return s.sign();
    }
    public boolean verify(byte[] data, byte[] sig, java.security.PublicKey pub) throws Exception {
        Signature s = Signature.getInstance(SIGN_ALGO);
        s.initVerify(pub);
        s.update(data);
        return s.verify(sig);
